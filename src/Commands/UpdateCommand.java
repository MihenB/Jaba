...
/**
 * Класс для обновления значения по id
 */
public class UpdateCommand {
    ...
    /**
     * Основной метод класса UpdateCommand, реализующий его смысл
     *
     * @param hashSet изменяемая коллекция
     * @param inputID id объекта, который нужно заменить
     * @param initDate дата инициализации
     * @param obj строковое представление объекта
     * @return изменённая коллекция
     */
    public LinkedHashSet<Worker> mainMethod(LinkedHashSet<Worker> hashSet, String inputID, Date initDate, String obj){
        try {
            ...
            List<Worker> listOfWorkers = new ArrayList<>(hashSet);
            while (i.hasNext()) {
                Worker worker1 = i.next();
                if (Objects.equals(worker1.getId(), id)) {
                    int index = listOfWorkers.indexOf(worker1);
                    listOfWorkers.set(index, worker);
                    hashSet = new LinkedHashSet<>(listOfWorkers);
                    break;
                }
            }
