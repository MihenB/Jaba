...
import java.util.*;
/**
 * Класс для добавления минимального элемента
 */
public class AddIfMinCommand {
    ...
    /**
     * Основной метод класса AddIfMinCommand, реализующий его смысл
     *
    ...
    public LinkedHashSet<Worker> mainMethod(LinkedHashSet<Worker> hashSet,Date initDate, String obj){
        try {
            ...
            for (Worker workerForSalary : hashSet) {
                listSalary.add(workerForSalary.getSalary());
            }
            Double minSalary = Collections.max(listSalary);
